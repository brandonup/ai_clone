...
# Modified to accept collection_name and delete_after_ingestion parameter
def ingest_document(file_content: Union[str, bytes], filename: str, collection_name: str, delete_after_ingestion: bool = True) -> Dict[str, Any]:
    """
    Ingest a document into the custom RAG system into a specific collection.
    ...
    try:
        ...
        if len(extracted_text) > 1000000:  # More than ~1MB of text
            ...
            # Pass the specific collection_name to QdrantStore
            store = QdrantStore(url=qdrant_url, api_key=qdrant_api_key, collection_name=collection_name)
            ...
            # Store document using the specific collection_name
            store = QdrantStore(url=qdrant_url, api_key=qdrant_api_key, collection_name=collection_name)
            # Pass the extracted_text to the processor
            processed_doc = processor.process_document(extracted_text, metadata)
            store.store_document(processed_doc) # store_document uses the collection_name set during init
    except Exception as e:
    ...
    if delete_after_ingestion and doc_path and os.path.exists(doc_path):
        try:
            os.remove(doc_path)
            logger.info(f"Deleted local copy of extracted text at {doc_path}")
        except Exception as e:
            logger.error(f"Error deleting local copy of extracted text at {doc_path}: {str(e)}")
